...
$(document).ready(function() {
	...
	midiPlayer = new MidiPlayer.Player((event) => {
		if (event.name == 'Note on' && event.velocity > 0) {
			samples.KEYBOARD.triggerRelease(event.noteName);
			samples.KEYBOARD.triggerAttack(event.noteName);
			// $.play(InstrumentEnum.KEYBOARD, event.noteName, true);
